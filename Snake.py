...
class Snake:
    def __init__(self, game, x, y, brain):
        ...
        for i in range(numberOfSenses):
            self.distToObstacles.append(0)
    ...
    def Update(self):
        ...
        if self.game.time - self.lastEaten > hungerTimeLimit*1000 and self.game.training:
    ...
    def Die(self):
        ...
        for part in self.body:
            foodPerPart = self.length/len(self.body)
            for i in range(math.ceil(self.radius/2)):
                ...
                self.game.world.DropFood(xPos,yPos, foodPerPart/math.ceil(self.radius/2))
