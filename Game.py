    ...
    def __init__(self):
        ...
        try:
            if self.loadFromFile:
                brains = []
                filePath = "savedBrains/1-" + str(numberOfBeams) + "-" + str(numberOfSenses)
                with open(filePath + "/savedBrain.obj", 'rb') as file_brain:
                    brain = pickle.load(file_brain)
                if brain.inputSize != numberOfBeams + numberOfSenses*2 + 1:
                    raise Exception
    ...
    def SpawnSnakes(self, brains = None, firstSpawn = 0):
        ...
        if brains == None: # first spawn
            for region in random.sample(self.world.regions, numberOfSnakes + self.isHumanPlayer):
                inNodes = numberOfBeams + numberOfSenses*2 + 1
                hiddenNodes = round((numberOfBeams + numberOfSenses*2 + 5)/2)
                outNodes = 2
                brain = NN.NeuralNetwork(inNodes, hiddenNodes, outNodes)
